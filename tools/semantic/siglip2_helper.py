#!/usr/bin/env python3
"""Timich semantic helper wrapper for the dev SigLIP 2 server."""
...
PROTOCOL_VERSION = 1
DEFAULT_SERVER_URL = "http://127.0.0.1:19185"
EXPECTED_RUNTIME = "transformers-siglip2"
...
def server_url() -> str:
    return os.environ.get("TIMICH_SIGLIP2_SERVER_URL", DEFAULT_SERVER_URL).rstrip("/")
...
def command_inspect(args: argparse.Namespace) -> int:
    ...
    if str(layout.get("runtime", "")).strip() != EXPECTED_RUNTIME:
        emit(identity_response(layout, False, False, "semantic_runtime_siglip2_layout_mismatch"))
        return 0
    try:
        emit(post_json("/inspect", {"layout": layout}, timeout=4.0))
    except (OSError, urllib.error.URLError, TimeoutError):
        emit(identity_response(layout, False, False, "semantic_runtime_siglip2_server_unavailable"))
    return 0
...
def command_embed_text(args: argparse.Namespace) -> int:
    layout = read_layout(args.runtime_layout)
    if str(layout.get("runtime", "")).strip() != EXPECTED_RUNTIME:
        print("SigLIP 2 text embedding failed: runtime layout is not transformers-siglip2", file=sys.stderr)
        return 1
    try:
        emit(post_json("/embed-text", {"layout": layout, "text": args.text}, timeout=30.0))
        return 0
    except (OSError, urllib.error.URLError, TimeoutError) as error:
        print(f"SigLIP 2 text embedding failed: {error}", file=sys.stderr)
        return 1
...
def command_embed_image(args: argparse.Namespace) -> int:
    layout = read_layout(args.runtime_layout)
    if str(layout.get("runtime", "")).strip() != EXPECTED_RUNTIME:
        print("SigLIP 2 image embedding failed: runtime layout is not transformers-siglip2", file=sys.stderr)
        return 1
    ...
    try:
        ...
        print(f"SigLIP 2 image embedding failed: {error}", file=sys.stderr)
        return 1
