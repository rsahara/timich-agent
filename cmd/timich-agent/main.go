...
func runCLI(args []string, stdout io.Writer, stderr io.Writer) error {
	...
	case "pre-release-migrate-catalog-v4-v5":
		return preReleaseMigrateCatalogV4ToV5(args[1:], stdout, stderr)
	default:
...
func loadConfigForServe(args []string, stderr io.Writer) (config.ResolvedConfig, error) {
	...
	if flags.NArg() != 0 {
		return config.ResolvedConfig{}, fmt.Errorf("unexpected argument %q", flags.Arg(0))
	}
